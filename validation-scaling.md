...
|----------|-----------|--------------|
| Камеры с видеоаналитикой | 180 | Число камер, для которых в реальном времени нужен инференс |
| Шаг инференса на камеру | 1 окно за 4 с (0,25 в секунду) | Инференс тяжёлый, поэтому анализируют не каждый кадр, а окна с шагом 4 секунды |
...
| Среднее число интерактивных обновлений на пользователя | 0,4 запроса в секунду | Ориентир: в пике пользователь запрашивает обновления в среднем раз в 2,5 секунды |

**Оценки вызовов и нагрузки (для оценки масштабирования):**

1. **Kafka** (телеметрия, датчики, нормализованные события) — сообщения в потоке могут исчисляться тысячами и десятками тысяч в секунду; это нельзя напрямую приравнивать к числу запросов к внешнему ML. Для Kafka в основном важно, чтобы потребители успевали: оценивают отставание и достаточность партиций.
2. **Вызовы внешнего ML по видео**:
   - среднее: `180 × 0,25 = 45` запросов в секунду
   - пик: `45 × 1,4 ≈ 63` запросов в секунду
3. **Прогноз трафика по сегментам**:
   - среднее: `600 / 60 = 10` запросов в секунду (если один прогноз относится к текущему периоду обновления)
   - пик: `10 × 1,4 = 14` запросов в секунду
4. **Синхронные запросы операторского контура к Core**:
   - среднее: `20 × 0,4 = 8` запросов в секунду
   - пик интерактивных обновлений: около `12` запросов в секунду
