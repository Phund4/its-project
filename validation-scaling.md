...
## Шаг 2. Анализ потерь данных

В этом шаге рассматриваются именно **потери данных**: когда часть событий не доходит до хранилища, приходит с задержкой, дублируется или становится непригодной для обработки.

**Телеметрия транспортных средств.** Потери возникают из-за нестабильной связи и перегрузки на приёме. В результате часть точек траектории отсутствует, и картина движения искажается. Для снижения риска нужны буферизация на источнике, повторная отправка, контроль порядка сообщений и мониторинг доли пропусков по каждому устройству.

**Дорожные датчики и описание сети.** Пропуски измерений, зависшие датчики и устаревшие справочники сегментов. Чтобы не терять качество расчётов, следует проверять полноту потока по каждому датчику, быстро выявлять «молчание» источника, а версии справочников фиксировать и обновлять по регламенту.

**Видео с камер.** Потери проявляются как пропущенные кадры, обрыв потока или ухудшение качества изображения. Это снижает надёжность распознавания событий. Практические меры: контроль доступности потока, проверка частоты кадров и разрешения, автоматический перезапуск подключения и оповещение при длительном отсутствии видео.

**Городские базы и архивы событий.** Возможны неполные выгрузки, задержки публикации и частичные обновления. Из-за этого в системе отсутствуют важные записи о событиях. Нужно вести журнал загрузок, проверять целостность выгрузок, повторять неуспешные загрузки и хранить дату среза для каждой партии данных.
...
- **Ограничение частоты запросов (rate limiting).** На входе в шлюз ставится предел обращений в секунду к внешнему API. Это защищает сервис от перегрузки в пике и не даёт одному источнику занять весь канал.
- **Повторы при временных сбоях (retries).** Если внешний вызов завершился ошибкой сети или таймаутом, выполняют 2-3 повторные попытки с увеличением паузы между ними. Это повышает устойчивость к кратковременным сбоям.
- **Ограничение времени ожидания (timeouts).** Для каждого внешнего вызова задают верхнюю границу ожидания ответа. Зависшие запросы не блокируют воркеры и не накапливают очередь.
...
Итого при анализе видео на 3 fps рост нагрузки закрывается сочетанием **увеличения реплик Go-сервисов в Kubernetes**, **разделов и потребителей Kafka**, **масштабирования кластера ClickHouse** по документации и **преимущественно отложенной обработки** вызовов внешнего API.
