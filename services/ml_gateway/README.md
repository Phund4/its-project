...
Тонкий HTTP-слой между **`ml_experiments`** и **`analytics`**: принимает `POST /v1/road-events`, валидирует JSON и **синхронно пересылает** то же тело в **`analytics`** (`ANALYTICS_BASE_URL` + `ANALYTICS_INGEST_PATH`). Локального хранения и **нет** `GET /v1/road-events`.
...
```bash
cd services/ml_gateway
go run ./cmd/gateway
```

Порядок: **ClickHouse** → **analytics** → **ml_gateway** → **ml_experiments** → **`data_ingestion`** (только для видео-потока).

В **`ml_experiments/.env`** по-прежнему **`ML_GATEWAY_URL`** на этот сервис.
