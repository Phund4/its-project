...
flowchart LR
  ...
  analytics -->|"REST/WebSocket: UI бизнеса"| ui
...
- **DataLake ↔ ML Gateway**  
  - К хранилищу обращаются **SQL-запросами**, но по сети это **HTTP(S) или нативный протокол ClickHouse** (см. таблицу 6.1.1).  
...
- **Analytics → Adapters / UI**  
  - **REST/gRPC** наружу — **команды и рекомендации** для дорожных систем.  
  - **HTTP + WebSocket** к UI — **состояние мониторинга трафика** и действия оператора.

- **Экспорт метрик → Monitoring; Grafana**  
  - Модули отдают **метрики в формате Prometheus** по **HTTP `/metrics`**.  
  - **Grafana** запрашивает у **Prometheus** (в составе Monitoring) данные через **Prometheus HTTP API** — на дашбордах видны те же показатели, что собирает мониторинг.  
  - **Оператор** открывает Grafana в браузере (**HTTPS**) и отдельно работает в **операционном UI** Core — это два разных интерфейса: Grafana для **инфраструктурных/технических метрик**, UI Core — для **бизнес-операций** (карта, инциденты, режимы).
