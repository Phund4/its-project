...
| Метрика | Формула (идея) | Что означает простыми словами |
|--------|----------------|-------------------------------|
| **MAE** (Mean Absolute Error) | Среднее по выборке от \|предсказание − факт\| | «В среднем модель ошибается на столько-то единиц» (например, на 5 км/ч по скорости). Все ошибки учитываются одинаково. |
| **RMSE** (Root Mean Squared Error) | Корень из среднего квадрата ошибок | То же, что «средняя величина ошибки», но **крупные ошибки влияют сильнее**. Полезен, когда редкие большие промахи нежелательны (например, резкое недооценивание затора). |
| **MAPE** (Mean Absolute Percentage Error) | Среднее от \|ошибка / факт\| × 100% | «В среднем модель ошибается на столько-то процентов». Удобно для сравнения качества на разных участках с разной загруженностью. |

**Метрики классификации и детекции (инциденты, нарушения, объекты на видео)**

| Метрика | Определение | Что означает простыми словами |
|--------|-------------|-------------------------------|
| **Precision** (точность) | TP / (TP + FP) | Из всего, что модель **назвала положительным** (инцидент/нарушение), какая доля **действительно положительная**. Высокий Precision → мало ложных тревог. |
| **Recall** (полнота) | TP / (TP + FN) | Из всех **реальных положительных** случаев какая доля **нашла модель**. Высокий Recall → мало пропущенных инцидентов. |
| **F1** | 2 × (Precision × Recall) / (Precision + Recall) | Один показатель, балансирующий Precision и Recall. Удобен, когда нужно не жертвовать ни точностью, ни полнотой. |
| **mAP** (mean Average Precision) | Усреднение Average Precision по классам и порогам IoU | Для детекции объектов на изображении/видео: насколько хорошо модель находит объекты (ТС, пешеходов) и правильно их ограничивает рамкой. Учитывает и «нашла/не нашла», и точность границ (IoU). |
...
- **Прогнозирование трафика**: малые MAE/RMSE/MAPE → точные прогнозы → эффективнее управление светофорами и маршрутизация → снижение времени в пути и задержек.
- **Детекция инцидентов**: высокий **Recall** → меньше пропущенных инцидентов (безопасность); достаточный **Precision** → меньше ложных тревог и доверие операторов; **mAP** отражает качество видеоаналитики для загруженности и нарушений.
...
flowchart LR
  ...
  dataStorage --> mlGateway
  mlGateway --> analytics
  analytics --> dataStorage

  mlService["ML-сервис (внешний, по API)"] <-->|"Запросы / ответы"| mlGateway

  operators["Операторы ИТС"] -->|"Запросы, настройки"| core
  core --> operators

  cityControl["Системы управления светофорами/инфраструктурой"] <-->|"Рекомендации, команды"| analytics

  citizens["Граждане и водители (приложения, навигаторы)"] <-->|"Маршрутизация, уведомления"| analytics
```

Объяснение:
- **В центре** — Core-сервис: приём данных, хранилище, вызов ML по API, аналитика, мониторинг, выдача рекомендаций.
- **Внешние акторы**: операторы ИТС, системы управления дорожной инфраструктурой, граждане (через приложения/навигаторы).
...
- **Откуда поступают данные для аналитики и инференса**:
  - **В Core-сервис**: телеметрия, показания датчиков, видеопотоки/кадры, данные из городских БД — через приём (Data Ingestion) и брокер сообщений; данные сохраняются в хранилище Core-сервиса.
...
- **Data Ingestion Service**:
  ...
  - публикация событий в брокер сообщений и/или запись в хранилище Core-сервиса.
...
- **Data Lake / DWH (хранилище Core-сервиса)**:
  ...
  - поддержка аналитических запросов, отчётности и выгрузки данных для дашбордов.
...
- **Analytics & Orchestration**:
  ...
  - агрегация и подготовка данных для UI и интеграций.

- **Monitoring & Alerting**:
  - мониторинг инфраструктурных метрик Core-сервиса (нагрузка, задержки, доступность);
  - мониторинг доступности и задержек вызовов ML-сервиса по API;
  - алерты для операторов и инженеров при сбоях или деградации.

- **User Interface / Dashboards**:
  - визуализация карты трафика, инцидентов, метрик и рекомендаций;
  - управление конфигурацией Core-сервиса и режимами работы (ручной/автоматический).
...
flowchart LR
  ...
  extCameras -->|"RTSP/HTTP"| dataIngest
  extSensors -->|"gRPC/HTTP"| dataIngest
  extCityDB -->|"REST/Batch"| dataIngest

  dataIngest -->|"Kafka topics"| broker
  broker -->|"Kafka consumer"| dataLake
  ...
  broker -->|"метрики брокера"| monitoring
```
...
- Альтернатива: **RabbitMQ** — проще, но хуже масштабируется для больших потоков; ориентирован на очереди задач.
...
- **HTTP/gRPC-клиент** (в составе приложения Core или отдельный микросервис) для вызова внешнего ML-сервиса:
  - использование **FastAPI** или аналога для единого API-слоя Core-сервиса; в его рамках — клиент к ML API (httpx, gRPC-клиент).
  - повторные попытки, таймауты, логирование запросов/ответов для отладки и мониторинга.
- Альтернатива: отдельный BFF/Proxy только для ML — усложняет архитектуру без необходимости при одном внешнем ML-сервисе.

Выбор: **FastAPI** как основной фреймворк Core-сервиса; вызовы ML — через встроенный клиент (REST или gRPC по контракту ML-сервиса).

#### 7.4. Аналитика и оркестрация (Core-сервис)

- **Ядро логики** на Python (или выбранном языке Core-сервиса): агрегация данных из хранилища, формирование рекомендаций, решение о том, когда и какие запросы отправлять в ML-сервис.
- **Планировщики/воркеры** для периодических задач (агрегация, вызов ML по расписанию): **Celery**, **Apache Airflow** или встроенные планировщики в приложении.
- Выбор **Airflow** или **Celery** — при необходимости сложных DAG и переиспользования ETL; для простых периодических вызовов достаточно планировщика в приложении.
...
- **Argo CD / GitOps** — управление релизами через Git.

Альтернатива: развёртывание на ВМ проще, но хуже масштабируется и усложняет поддержку нескольких компонентов.
