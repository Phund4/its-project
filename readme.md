...
|--------|----------------|-------------------------------|
| **MAE** (Mean Absolute Error) | Среднее по выборке от \|предсказание − факт\| | **Средняя ошибка в тех же единицах**, что и показатель. Например, MAE = 5 по скорости → в среднем промах на 5 км/ч. |
| **RMSE** (Root Mean Squared Error) | Корень из среднего квадрата ошибок | **Как MAE, но сильнее наказывает большие ошибки**. Например, несколько крупных промахов заметно поднимут RMSE. |
| **MAPE** (Mean Absolute Percentage Error) | Среднее от \|ошибка / факт\| × 100% | **Средняя ошибка в процентах**. Например, MAPE = 10% → в среднем модель ошибается на 10% от реального значения. |

**Метрики классификации и детекции (инциденты, нарушения, объекты на видео)**

| Метрика | Определение | Что означает простыми словами |
|--------|-------------|-------------------------------|
| **Precision** (точность) | TP / (TP + FP) | **Из всех срабатываний модели, какая доля верных**. Например, из 100 тревог 90 настоящие → precision = 0.9. |
| **Recall** (полнота) | TP / (TP + FN) | **Из всех реальных событий, сколько нашла модель**. Например, было 100 ДТП, модель нашла 80 → recall = 0.8. |
| **F1** | 2 × (Precision × Recall) / (Precision + Recall) | **Одна цифра, которая одновременно учитывает precision и recall**. Высокий F1 значит мало пропусков и мало ложных тревог. |
| **mAP** (mean Average Precision) | Усреднение Average Precision по классам и порогам IoU | **Насколько хорошо в среднем модель находит и обводит объекты** на картинке/видео. Чем выше mAP, тем точнее детекция и рамки. |
